...
import { readThemeColor, reducedMotionQuery } from "../canvas";
...
export function VerletClothCanvas({
  ...
  useEffect(() => {
    ...
    let animId: number;
    const reduceMotion = reducedMotionQuery();
    let W = 0, H = 0;
    const COLS = 7, ROWS = 7;
    ...
    function render(now: number) {
      if (!ctx || !canvas || W === 0 || particles.length === 0) {
        // Not measurable yet (or hidden): retry next frame rather
        // than ending the animation loop for good.
      ...
      const themeColor = readThemeColor(canvas);
      ...
      ctx.restore();
      if (!reduceMotion.matches) animId = requestAnimationFrame(render);
    }
