import { CurveConfig } from "./useCurveAnimation";
...
/** Every preset id, in gallery order. Also the `cluster-loaders add <id>` argument. */
export const loaderIds: string[] = curves.map((curve) => curve.id);

/** Look a preset up by its stable id. */
export function curveById(id: string): CurveConfig | undefined {
